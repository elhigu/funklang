// Op 9 — add(short val1, short val2) → clamp(val1 + val2)
//
// Source: exe_creator/synthnodes.h line 100-102.
// Args (per refrender.c case 9):
//   val1 must be v1..v4 (literal not allowed); else 0
//   val2 = pick_short(slot.val2, slot.val2Value, vars)

import type { OpFn } from '../types';
import { pickShort, clampI16 } from './_helpers';

export const op_add: OpFn = (_state, _slotIdx, vars, slot, _t) => {
  const a = (slot.val1 > 0 && slot.val1 <= 4) ? vars[slot.val1]! : 0;
  const b = pickShort(slot.val2, slot.val2Value, vars);
  return clampI16(a + b);
};
