...
export type ParamKind =
  | { kind: 'const-int';    min: number; max: number; label: string }
  | { kind: 'var-source';   label: string; allowNone?: boolean }
  | { kind: 'var-or-const'; min: number; max: number; label: string }
  ...
  // Gain selector, range 0..128.
  {
    ...
    params: [
      { field: 'freqVal', selector: 'freq',
        type: { kind: 'var-or-const', min: 0, max: 10000, label: 'freq' } },
      { field: 'gainVal', selector: 'gain',
