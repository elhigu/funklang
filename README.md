...
- **No sample-import UI yet.** funklang's focus so far is synthetic, code-generated
  instruments. Samples that are already in a loaded `.akp` are fully supported —
  read, played (the `imported_sample` op), kept on save, and emitted in the exported
  asm/C/`.bin`. What isn't built yet is importing a *new* sample in the editor
  (wav / raw / mic); that's an early-scope choice, not a fundamental one, and it may
  well be added later.
