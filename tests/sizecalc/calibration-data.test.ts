import { describe, it, expect } from 'vitest';
import { CALIBRATION } from '../../src/sizecalc/calibration-data';
import { OP_DEFS } from '../../src/schema/op-metadata';

describe('CALIBRATION seed table (anchored power-law model)', () => {
  it('has the required top-level fields', () => {
    expect(typeof CALIBRATION.floor).toBe('number');
    expect(typeof CALIBRATION.perDistinctOp).toBe('number');
    expect(typeof CALIBRATION.perSlotPow).toBe('number');
    expect(typeof CALIBRATION.slotPower).toBe('number');
    expect(typeof CALIBRATION.perVarOperand).toBe('number');
    expect(typeof CALIBRATION.modLengthEmpty).toBe('number');
    expect(typeof CALIBRATION.fitted).toBe('boolean');
  });

  it('has an opWeight entry for every known op code', () => {
    for (const def of OP_DEFS) {
      expect(CALIBRATION.opWeight[def.code], `op ${def.code} (${def.name})`).toBeTypeOf('number');
    }
  });

  it('is calibrated from the corpus with sane coefficients', () => {
    expect(CALIBRATION.fitted).toBe(true);
    expect(CALIBRATION.floor).toBeGreaterThan(0);
    expect(CALIBRATION.perDistinctOp).toBeGreaterThan(0);
    expect(CALIBRATION.perSlotPow).toBeGreaterThan(0);
    expect(CALIBRATION.slotPower).toBeGreaterThan(0);
    expect(CALIBRATION.slotPower).toBeLessThanOrEqual(1); // concave (sub-linear) in slots
    expect(CALIBRATION.perVarOperand).toBeGreaterThan(0);
    for (const def of OP_DEFS) expect(CALIBRATION.opWeight[def.code]).toBeGreaterThanOrEqual(0);
  });

  it('records leave-one-out fit residuals', () => {
    expect(CALIBRATION.fit.meanErr).toBeGreaterThanOrEqual(0);
    expect(CALIBRATION.fit.maxErr).toBeGreaterThanOrEqual(CALIBRATION.fit.meanErr);
    expect(CALIBRATION.fit.meanPct).toBeGreaterThanOrEqual(0);
  });
});
