...
- [ ] **Step 2 — framework.** Port `Main`'s fixed scaffolding (`../reference/.../Program.cs` ~76–465 minus per-op dispatch) into `framework.ts` + `index.ts`: equ block (compute `AK_SMP_LEN`=Σ sampleLengths, `AK_EXT_SMP_LEN`=Σ imports, `fineProgressLength`), prologue (sample/ext base-address loops), per-instrument header+`.InstNLoop`+tail, epilogue, `AK_ResetVars`, `VarsCode`. Helpers in `helpers.ts`. Use `useProgress=useFineProgress=true` to match the oracle default.
